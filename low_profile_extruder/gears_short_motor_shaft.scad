...
gear2_bolt_hex_d = 8.1;		// was 12.8 for Greg/Wade
...
module gearsbyteethanddistance(t1=13,t2=51, d=60, teethtwist=GEAR_TWIST,which=1)
{
	...
	if(which == 1)
	{
		...
		difference()
		{
			...
			//DIFFERENCE:
			//shafthole
			translate([0,0,gear_h + gear_shaft_h - gear_short_shaft_h + 2*TT]) 
				cylinder(r=gear1_shaft_r, h=gear_short_shaft_h, $fn=32);
	...
	else
	{
		...
		difference()
		{
			...
			// sunken face
			translate([0, 0, gear2_sunken_face])
				difference() {
					cylinder(r = g2p_r - gear2_thickness
, h=gear_h, center=true, $fn=64);
